...
if(!empty($_GET)){
    ...
    $stmt->execute();
    $onlyfuncionario= $stmt->fetch();// retorna apenas a linha referente ao id
    }
    else
    {
    ...
    $stmt->execute();
    $Allfuncionario=[];
    $Allfuncionario= $stmt->fetchAll();// retorna todas as linhas para a variavel
    }
...
$senha= $data["senha"];
$obs= $data["observacao"];
$query= "INSERT INTO funcionario (nome, sobrenome, cpf, emissor, data_nascimento, telefone, email, admissao, /*demissao,*/ cargo, senha, observacao) VALUES (:nome, :sobrenome, :cpf, :emissor, :data_nascimento, :telefone,
...
try{
$idDelete= $data["id"];
$query= "DELETE FROM funcionario WHERE id=:idDelete";
$stmt= $conn->prepare($query);
